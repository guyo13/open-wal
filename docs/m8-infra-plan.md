...
1. `storage-check.sh probe-write <wal_dir>` — write a marker **without** `fdatasync` (it must sit in the page cache / device cache, not on stable flash).
2. Cut power via the smart plug; restore; wait for boot.
3. `storage-check.sh probe-verify <wal_dir>` — the marker **MUST be absent** (exit 0 = gone; exit 1 = survived).
