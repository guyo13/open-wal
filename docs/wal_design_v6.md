...
> 2. **Secondary (behavioral power-loss) — a synchronized mid-run cut; NOT yet reproduced on any Linux config tested.** `scripts/m8/dm-flakey.sh dirfsync-negative <fs>` with `src/bin/dirfsync_cut_workload.rs`: roll once, ack a record into the new segment, then **block** with the dirent dirty so the harness activates `drop_writes` and cuts *inside* the un-synced window (not a sub-ms race). Empirically (PR #21, owner Fedora 43) the inject build **still recovers fully** on every config tried — the new segment's dirent reaches disk transitively via the file's own `fdatasync`. **Mechanism not isolated, and the earlier "ext2 block-adjacency" guess is retracted:** the dmesg shows a plain `ext2`-format volume is, on modern kernels (the standalone ext2 *driver* was removed in Linux 6.9), serviced by the **ext4 driver journal-less** ("mounting ext2 file system using the ext4 subsystem"), so a real ext2 driver was never exercised — the masking there is the ext4 driver's metadata/writeback, on journaling FSes it is the journal. One bounded attempt remains: journal-less ext4 is moot (it masks it), so the lever is the ext4 driver's **weakest ordering**, a *journaled* ext4 mounted `data=writeback` (`dirfsync-negative ext4-writeback`). If that too doesn't reproduce, the behavioral form is a **documented negative result** — not a gap, since Tier-1 carries the gate.
...
- §14.4d negative control catches the injected bug **and** the correct build passes. *(M8 — **satisfied by Tier-1.** **Tier-1 (primary) PASSES, deterministic + per-PR:** `scripts/m8/dirfsync-presence.sh` (in `ci.yml`) straces the roll path and asserts the correct build issues the roll-time directory `fsync` while `--features inject_no_dir_fsync` does not — verified green (`correct=5` dir-fsyncs vs `inject=1`). FS-independent syscall-presence regression guard; the row's satisfier. **Tier-2 (behavioral power-loss) — NOT reproduced on any Linux config tested (PR #21, owner Fedora 43):** the synchronized mid-run cut (`dirfsync_cut_workload`, `dirfsync-negative <fs>`) blocks the workload with the new segment's dirent un-synced and cuts inside the window, yet the inject build recovers fully everywhere — the dirent reaches disk via the file's own `fdatasync`. The earlier "ext2 exposes it / block-adjacency" claim is **retracted**: dmesg shows `ext2`-format is serviced by the **ext4 driver journal-less** on modern kernels (standalone ext2 driver removed in Linux 6.9), so masking there is the ext4 driver's, on ext4/xfs/btrfs it is the journal. One bounded attempt left — journaled ext4 `data=writeback` (the driver's weakest ordering, `dirfsync-negative ext4-writeback`); if it too masks, **finalize as a documented negative result** (honest close, not a gap). **Tier-3 — ext4/xfs/btrfs (+ journal-less "ext2") INCONCLUSIVE-by-design**, never red on a masked miss. `fsync_dir` retained unconditionally as a POSIX-portability safeguard. Earlier "certified on ext4" was wrong; the harness loud-skips where dm-flakey is absent rather than fake green; the positive split+roll power-loss case passes under LazyFS in M4.)*
