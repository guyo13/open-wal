...
- §14.4d negative control catches the injected bug **and** the correct build passes. *(M8: the **harness is built** — `scripts/m8/dm-flakey.sh dirfsync-negative` drives the correct vs `--features inject_no_dir_fsync` builds through a `drop_writes` simulated power loss and asserts the asymmetry — but it requires §14.8 metadata-fault tooling (dm-flakey/power-pull) that this sandbox's kernel lacks (`CONFIG_BLK_DEV_DM` absent). It is **OPEN-pending-owner-run**, certified on **ext4** (FS-dependent and timing-sensitive — see `docs/m8-runbook.md` §14.4d); the harness prints a loud "NOT EXERCISED" banner rather than fake green. The correct-build positive split+roll power-loss case already passes under LazyFS in M4.)*
...
- §14.8 H1: ≥ M power-pull cycles on target hardware, zero acked-record loss. *(M8: the **harness + runbook are built** — `src/bin/power_pull_{workload,verify}.rs` + `scripts/m8/power-pull.sh`, with the off-box network side channel, send-strictly-after-`commit() Ok` ack-ordering, contiguous-watermark conservative verify, and the H2 vacuous-pass gate as a precondition; the mechanical chain was dry-run green on loopback. **OPEN-pending-owner-run** for the actual ≥50-cycle power-pull on real/cache-configured hardware (no cuttable target in the sandbox). H3 fsync-failure poison: the **§12 state machine RUNS green** via the LD_PRELOAD shim (`scripts/m8/fsync-fault.sh`); the **physical** dm-flakey half is OPEN-pending-owner-hardware. H4 macOS `F_FULLFSYNC`: OPEN-pending-macOS. See `docs/m8-runbook.md`.)*
