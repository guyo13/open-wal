...
> 2. **Secondary (behavioral power-loss) — ext2 only, via a synchronized mid-run cut.** `scripts/m8/dm-flakey.sh dirfsync-negative ext2`. ext2 is journal-less, so `fdatasync(segment)` does **not** transitively persist the parent dirent ⇒ a `drop_writes` cut orphans the rolled segment in the inject build (recovery's `readdir` misses it ⇒ post-roll records lost) while the correct build's `fsync_dir` keeps it. The cut must land **inside the un-synced window**: `src/bin/dirfsync_cut_workload.rs` rolls once, acks a record into the new segment, and **blocks** with the dirent dirty so the harness cuts at leisure (a run-to-completion cut does *not* reproduce it even on ext2 — the dirent ages out and is written back first; observed PR #21). This is *the* substrate where the behavioral asymmetry is observable. Phase-1 status: **expected to reproduce — verify** (observe a first PASS before hard-gating).
> 3. **Documented INCONCLUSIVE-by-design — ext4/xfs/btrfs.** Run for evidence and to catch a genuine correct-build regression, but a non-failing inject build there is **expected** (journaling masks the omission), never read as "dir-fsync omission is harmless."
>
> *(Superseded the earlier "dm-flakey makes the buggy build fail on ext4" expectation, which was backwards — ext4 is where it is least observable. The `inject_no_dir_fsync` toggle drives both Tier-1 and Tier-2. §14.12/§14.13 track the DoD.)*
...
- §14.4d negative control catches the injected bug **and** the correct build passes. *(M8, three-tier — see §14.4d. **Tier-1 (primary) PASSES and is deterministic + per-PR:** `scripts/m8/dirfsync-presence.sh` (in `ci.yml`) straces the roll path and asserts the correct build issues the roll-time directory `fsync` while `--features inject_no_dir_fsync` does not — verified green (`correct=5` dir-fsyncs vs `inject=1`). This is the syscall-presence regression guard, FS-independent, and is the row's primary satisfier. **Tier-2 (behavioral power-loss) — ext2:** `scripts/m8/dm-flakey.sh dirfsync-negative ext2` via `.github/workflows/m8-dmflakey.yml` (nightly + dispatch on hosted VMs that reach dm-flakey); **Phase-1 OPEN — expected to reproduce, pending a first observed PASS** (the behavioral asymmetry is only observable on a journal-less fs). **Tier-3 — ext4/xfs/btrfs INCONCLUSIVE-by-design** (journaling transitively persists the new dirent on the segment's own fsync, masking the omission — AFSNCE OSDI '14, §18); run for evidence + correct-build-regression catching, never red on a masked miss. The `fsync_dir` is retained unconditionally as a portable-durability safeguard. Earlier "certified on ext4" was incorrect (ext4 masks it); superseded by this split. The harness loud-skips where dm-flakey is absent rather than fake green; the correct-build positive split+roll power-loss case already passes under LazyFS in M4.)*
