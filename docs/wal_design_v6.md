...
- **`loom` harness for the integration barrier (recommended, integrator-facing).** Model the journal-consumer→publish-consumer handoff; prove that under all interleavings the publish side never treats a record as publishable before the corresponding `commit` returned, given Release store / Acquire load on the shared cursor. *(**M9 — IMPLEMENTED** as `tests/loom_publish.rs` (`#![cfg(loom)]`). A writer thread commits each record (writes its payload) then advances a watermark `AtomicUsize` with a **Release** store; a publish-consumer thread **Acquire**-loads the watermark and asserts every record at or below it carries its committed payload — i.e. the publish side never runs ahead of the commit whose bytes it would ship. `loom::model` explores all interleavings/reorderings. **Falsifiability shown:** weakening the watermark store to `Relaxed` makes loom find an interleaving where the consumer sees `watermark=1` but the payload is still unwritten (`record 1 was published before its commit was visible`), then reverted. **It is integrator-facing — it does not touch WAL internals**; it pins the correct ordering the `on_durable(durable_lsn)` contract (§15.3) assumes. `loom` is a `cfg(loom)`-only dev-dependency (absent from normal / release / MSRV builds); run with `RUSTFLAGS="--cfg loom" cargo test --test loom_publish`. CI: a dedicated `loom.yml` (paths-filtered PR + push-to-main + nightly + dispatch), off the per-PR hot path per §14.6.)*
...
- Integrator publish barrier modeled under `loom` (§14.6 / §15.3, recommended). *(**M9 — DONE.** `tests/loom_publish.rs` (`#![cfg(loom)]`) models the Release-store-watermark / Acquire-load-watermark handoff and asserts the publish side never observes a record as publishable before its commit is visible; `loom::model` is exhaustive over interleavings. Falsifiability shown (weaken store to `Relaxed` ⇒ loom finds a stale read). Integrator-facing — does not touch WAL internals. `loom` is a `cfg(loom)`-only dev-dep; CI `loom.yml` (paths-filtered PR + push-to-main + nightly + dispatch), off the per-PR hot path.)*
