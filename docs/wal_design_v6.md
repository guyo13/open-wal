...
- **§14.10 / §14.11 — soak / endurance test IMPLEMENTED (`tests/soak.rs`, `scripts/m9/soak.sh`, `soak.yml`).** A single long-lived `Wal` driven through a sustained weighted append/commit/`checkpoint(durable)`/crash-recover workload over a small (4 KiB-segment/256 B-record) config, re-checking the §14.3 durability envelope (D1/D2/D3/D6/D8) against an independent oracle after every recover, with bounded-growth gates on fd count / segment-dir disk / RSS / commit-latency p999. Deterministic (seeded LCG), env-driven duration. Nightly/dispatch CI lane (informational/contingent — a multi-hour run on a dedicated runner is the actual §14.13 gate; a hosted slice is a smoke). No `src/` change (public API only). (Added during M9.)
- **§14.6 — removed the `loom`-harness item as not applicable in v1.** A loom harness models concurrent shared-memory access, but the writer is `!Sync` (§6.2, enforced by the compile-fail test), so the single-writer property is proven *statically* (stronger than a model check) and there is **no intra-WAL shared-memory concurrency to model**. The only Release/Acquire publish barrier is the **integrator's** journal cursor, which is out of scope (§1, §15.7); the WAL's observer-ordering obligation is covered by the §15.8 `DurabilityObserver` contract tests. The item becomes applicable only if the writer is ever made `Sync`. (A v1 attempt — PR #30, closed unmerged — confirmed the point: the harness could only be written by re-implementing the integrator's cursor inside the test, with no `open_wal` linkage. Same discipline as the §14.4d negative result: the characterized "why not" is the deliverable, not a silent deletion. Found during M9.)
...
- **Concurrency:** single-writer (`&mut self`, `!Sync`, dir lock). No threads, no background I/O. Cross-thread cursor coordination (the publish barrier) is the integrator's and out of scope (§1, §15.7); because the writer is `!Sync` there is no intra-WAL shared state to model with `loom` (§14.6 — the single-writer property is proven statically instead).
...
- **loom (integration barrier) — not applicable while the writer is `!Sync`.** A loom harness models concurrent shared-memory access; the WAL writer is `!Sync` (§6.2, enforced by the compile-fail test above), so there is no intra-WAL shared state for loom to explore — the single-writer property is proven *statically*, which is stronger than a model check. The only Release/Acquire publish barrier is the **integrator's** journal cursor, which is out of scope (§1, §15.7). The WAL's sole obligation at that boundary — `DurabilityObserver::on_durable` fires strictly after durability, monotonically, on the writer thread — is covered by the §15.8 observer-contract tests. *(Were the writer ever made `Sync` — e.g. a future multi-writer or interior-mutability redesign — a loom harness would become applicable and should be added then. A v1 attempt confirmed the point: the only way to write the test was to re-implement the integrator's cursor inside the test, since the WAL exposes no shared surface to model — PR #30, closed unmerged.)*
