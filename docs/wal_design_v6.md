...
- **§14.8/§14.11 — M8 test AUTOMATION added (Tier 1 dm-flakey CI + Tier 3 macOS CI); no contract change.** The two metadata/physical gates the build sandbox could not run are now nightly + manual CI: `.github/workflows/m8-dmflakey.yml` runs **H3-physical (#16)** and **§14.4d (#17)** via `dm-flakey` on hosted `ubuntu-latest` VMs (which, unlike the build sandbox, reach the target) — ext4 is the hard gate (FAIL reds the build), xfs/btrfs informational, **best-effort + loud skip** if a runner lacks dm-flakey; and `.github/workflows/m8-macos.yml` runs **H4 Half A (#19)** (`cargo test --test macos_fullfsync`, routing/smoke) on `macos-latest` (Half B `dtruss` stays owner-run). The amended anti-vacuous acceptance criteria are enforced **inside** `scripts/m8/dm-flakey.sh`: #16 asserts the EIO actually reached a commit (injection-fired, bounded retry; clean/no-injection = INCONCLUSIVE, never PASS), #17 grants a bounded retry budget for the timing-sensitive asymmetry (exhausted = INCONCLUSIVE, never PASS; a correct-build loss = FAIL). Every gate emits a §5 evidence-ledger JSON (`scripts/m8/evidence.sh`) uploaded as a CI artifact each run and posted to the tracking issue **only on a manual `workflow_dispatch`** (the human sign-off; nightly stays loud as a red build, quiet on the issue). H1 power-pull (#18) remains owner-run. No `src/` change. (Added during M8 test-automation.)
...
- §14.4d negative control catches the injected bug **and** the correct build passes. *(M8: the **harness is built** — `scripts/m8/dm-flakey.sh dirfsync-negative` drives the correct vs `--features inject_no_dir_fsync` builds through a `drop_writes` simulated power loss and asserts the asymmetry — but it requires §14.8 metadata-fault tooling (dm-flakey/power-pull) that this sandbox's kernel lacks (`CONFIG_BLK_DEV_DM` absent). It is certified on **ext4** (FS-dependent and timing-sensitive — see `docs/m8-runbook.md` §14.4d). **M8 test-automation:** `.github/workflows/m8-dmflakey.yml` (nightly + `workflow_dispatch`) now drives this on hosted `ubuntu-latest` VMs, which — unlike the build sandbox — reach `dm-flakey`; the harness enforces the amended anti-vacuous criterion (bounded retry budget; exhausted = INCONCLUSIVE, never PASS) and **best-effort + loud skip** if a runner lacks the target. The release-gate proof is that workflow's first green run (or a manual `workflow_dispatch` sign-off posting its §5 evidence to #17); until observed it is **contingent**, and a real power-pull remains the ultimate authority. The harness still prints a loud "NOT EXERCISED" banner rather than fake green where dm-flakey is absent. The correct-build positive split+roll power-loss case already passes under LazyFS in M4.)*
