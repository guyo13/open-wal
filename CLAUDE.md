...
- **Current milestone:** M3 (intra-segment recovery) — **Phase 1 complete (recovery code + all non-FUSE fault tests); the M3 LazyFS gate (Phase 2) is still OPEN, so M3 is NOT done.** New module `src/recovery.rs` implements §8.2: `recover_segment(file, base_lsn, is_active, …)` scans the active segment's dense run, detects a torn tail, and on truncation **durably zeros `[X, EOF)` via `segment::zero_to_eof` (`pwrite`-zeros + `sync_data_fully`, §8.2.1)** so nothing stale can be resurrected (D10); a **bounded forward scan** (`max_record_size + 28`, 8-aligned from `X+8`) classifies mid-log corruption as fatal `TornMidLog` (D5) and the `is_active=false` path makes any invalid record in a sealed segment fatal `Corruption`. `src/segment.rs`: the scanner now returns `ScanOutcome::{Record, CleanEnd, Invalid}` (M2's collapsed `End` split into the §8.2 step-1 clean-end vs the step-2–4 candidate boundary), tolerates short physical reads (`read_full_at`, for truncated files — D11), and gained `zero_to_eof`. `src/wal.rs::open` now runs `recovery::recover_segment` on the active segment and reports a real `TailState::TruncatedAt`; a truncated-below-64 header maps to `BadSegmentHeader` not raw `UnexpectedEof`. Tests: `src/recovery.rs` unit tests (torn-tail-zeroing, mid-log fatal, sealed fatal, idempotence, truncated-file, **the precise stale-LSN==expected resurrection/D10 case**, interim D11 arbitrary-bytes), plus integration suites `tests/recovery_corruption.rs` (§14.4e), `tests/recovery_truncation.rs` (§14.4f), `tests/recovery_resurrection.rs` (§14.4g logic), and `tests/process_crash.rs` (§14.4a, SIGKILL via the test-only `src/bin/crash_child.rs`). `cargo test`, `cargo clippy --all-targets -D warnings`, `cargo fmt --check` all green. **Deliberately still NOT done:** the LazyFS gate (§14.4b lost/torn writes + §14.4g **durability-of-zeroing** under power-loss) — Phase 2, must run where FUSE exists; segment roll / multi-segment / commit-time split + the §8.4 discard of an incomplete-header highest-base file + cross-segment continuity §8.1 (M4); checkpoint (M5). M3's `open` still recovers only the single active segment. (M0/M1/M2 remain complete.)
...
- **M3 gate:** NOT yet passed — Phase 1 (recovery code + non-FUSE tests §14.4a/e/f/g-logic) done; Phase 2 (LazyFS §14.4b + §14.4g durability-of-zeroing) still pending and must run where FUSE/privileges exist.
