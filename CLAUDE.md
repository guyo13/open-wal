...
- **Current milestone:** M2 — complete (single-segment write/read). New modules: `src/observer.rs` (`DurabilityObserver`/`NullObserver`, the §15.3 hook, in core), `src/segment.rs` (§5.2 header + CRC, `{base_lsn:020}.wal` filenames, `fallocate` pre-alloc + create, `sync_data_fully` = `F_FULLFSYNC` on macOS / `sync_data` elsewhere, and the shared bounded record scanner), `src/reader.rs` (lending streaming `Reader`, §6.1), `src/wal.rs` (`Wal<O: DurabilityObserver = NullObserver>` — `Send`+`!Sync` via `PhantomData<Cell<()>>`, exclusive `flock`, `open`/`open_with`, pure-memory `append` §7.1, single-segment `commit` §7.2 with poison-on-fsync-failure §12, `durable_lsn`/`last_lsn`/`reader_from`, cold-start + clean single-segment reopen). §14.1 unit tests (segment header, filename↔base_lsn incl. 1/`u64::MAX`, §5.3 config bound, max-record-fits, `RecordTooLarge`, `Locked`), §14.2 property tests P1/P2/P3/P6 (`tests/single_segment.rs`), and the §14.7 zero-alloc assertion for `append`+`commit` and `Reader::next` (`tests/zero_alloc.rs`) all pass; `cargo test`, `cargo clippy --all-targets -D warnings`, `cargo fmt --check` green. **M3 (intra-segment recovery) is next.** **Deliberately NOT in M2** (so as not to fake later gates): segment roll / multi-segment / commit-time split (M4), torn-tail detection + zero-to-EOF + bounded forward scan + mid-log-corruption classification + sparse index (M3), checkpoint (M5). `open`'s reopen handles only the **clean** single-segment case and always reports `TailState::Clean`; `commit` assumes the batch fits one segment (debug-asserted). (M0/M1 — `Lsn`/`WalConfig`/`WalError`, CRC-32C, and the `src/record.rs` codec — remain complete.)
